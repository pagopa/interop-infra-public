resource "aws_sqs_queue" "uptime_cost_optimization_dlq" {
  ...
  name = format("%s-analytics-eventbridge-uptime-cost-optimization-dlq-%s", local.project, var.env)

  message_retention_seconds = 1209600 # 14 days
  max_message_size          = 262144  # 256 KB
}

resource "aws_iam_policy" "redshift_uptime_cost_optimization" {
  count = local.deploy_uptime_cost_optimization ? 1 : 0

  name = "InteropAnalyticsRedshiftUptimeCostOptimization"
...
resource "aws_iam_role" "uptime_cost_optimization" {
  ...
  name = format("%s-analytics-eventbridge-uptime-cost-optimization-%s", local.project, var.env)
  ...
  managed_policy_arns = [aws_iam_policy.redshift_uptime_cost_optimization[0].arn]
}

resource "aws_scheduler_schedule_group" "uptime_cost_optimization" {
  count = local.deploy_uptime_cost_optimization ? 1 : 0

  name = format("%s-analytics-uptime-cost-optimization-%s", local.project, var.env)
}

resource "aws_scheduler_schedule" "start_redshift_working_hours" {
  count = local.deploy_uptime_cost_optimization ? 1 : 0

  name       = format("%s-start-redshift-working-hours-%s", local.project, var.env)
...
resource "aws_scheduler_schedule" "stop_redshift_after_working_hours" {
  count = local.deploy_uptime_cost_optimization ? 1 : 0

  name       = format("%s-stop-redshift-after-working-hours-%s", local.project, var.env)
  group_name = aws_scheduler_schedule_group.uptime_cost_optimization[0].name
...
resource "aws_cloudwatch_metric_alarm" "sqs_uptime_cost_optimization_dlq" {
  ...
  alarm_name          = format("sqs-analytics-uptime-cost-optimization-dlq-has-messages-%s", var.env)
  alarm_description   = "DLQ for Analytics Uptime Cost Optimization has messages, it means there are failed schedules"
